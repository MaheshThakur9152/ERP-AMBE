...
export const PayrollHub: React.FC = () => {
  ...
  const loadPayrollData = async () => {
    ...
    try {
      ...
      initialRows.sort((a, b) => {
        const postA = (a.designation || 'Unassigned').trim().toUpperCase();
        const postB = (b.designation || 'Unassigned').trim().toUpperCase();
  ...
  // Unique designations / posts for segregation filter
  const availablePosts = useMemo(() => {
    const postSet = new Set<string>();
    rows.forEach((r) => {
      if (r.designation) postSet.add(r.designation.trim());
    });
    return Array.from(postSet).sort((a, b) => a.localeCompare(b));
  }, [rows]);

  const displayedRows = useMemo(() => {
    if (selectedPostFilter === 'all') return rows;
    return rows.filter((r) => (r.designation || 'Unassigned').trim() === selectedPostFilter);
  }, [rows, selectedPostFilter]);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          ...
          <div className="flex items-center border border-gray-300 rounded-xl px-3 py-2 bg-white text-xs font-semibold text-gray-800 shadow-xs gap-2">
            ...
            >
              <option value="all">All Posts ({rows.length})</option>
              {availablePosts.map((post) => {
                const count = rows.filter((r) => (r.designation || '').trim() === post).length;
                return (
            ...
            <table className="w-full text-left text-xs text-gray-700 border-collapse min-w-[2150px]">
              ...
              <tbody className="divide-y divide-gray-100 font-mono text-[11px]">
                {displayedRows.map((row, idx) => {
                  const currentPost = (row.designation || 'Unassigned').trim();
                  const prevPost = idx > 0 ? (displayedRows[idx - 1].designation || 'Unassigned').trim() : null;
                  const isNewGroup = idx === 0 || currentPost.toUpperCase() !== prevPost?.toUpperCase();
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] text-slate-500 font-medium">
                                  — {postStaffCount} {postStaffCount === 1 ? 'staff member' : 'staff members'}
                                </span>
                              </div>
                            </div>
                          </td>
              ...
              <tfoot className="bg-slate-100 border-t-2 border-gray-300 font-mono font-bold text-xs">
                <tr>
                  <td colSpan={11} className="p-3 text-right uppercase tracking-wider font-sans text-gray-600">
                    Grand Totals ({displayedRows.filter((r) => r.calc).length} Staff):
                  </td>
